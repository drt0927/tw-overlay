...
### 4. 업데이트 히스토리 (v1.11.4 패치)
- **신규:** 모험 일지 월간 수익 추이 그래프 및 자동 수익 기록 축약(아코디언) 기능 추가.
- **개선:** DB 스키마 확장(`amount` 컬럼)을 통한 통계 쿼리 성능 대폭 향상.
- **수정:** 멀티 모니터 좌표 이탈 및 DPI 스케일링 관련 위치 오류 해결.
- **보완:** 통계 탭 비활성화 시 차트 렌더링 에러 방지(Safety Check) 추가.
