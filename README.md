...
![Version](https://img.shields.io/badge/version-2.1.2-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v2.1.2 (2026.07.12)
이번 업데이트는 커스텀 알림 사운드 관리 서브 탭 신설, 알림 기록 24시간 청소 갱신 및 장비 사전 데이터 교정을 진행한 **패치 v2.1.2 업데이트**입니다.

- **🎵 커스텀 알림 사운드 관리 서브 탭 신설**:
  * 사용자가 원하는 사운드(.mp3, .wav 등)를 직접 등록하여 개별 알람음에 지정할 수 있는 관리 화면을 환경설정의 독립된 가로 서브 탭으로 새로 구축했습니다.
  * 등록한 커스텀 음원은 Roaming AppData 내부에 안전하게 백업 복사되어 원본이 유실되어도 무방하며, 다른 PC 등에서 물리 파일이 누락되어 로드하지 못하는 특수 상황에는 자동으로 기본 알림음(`orb.mp3`)으로 즉각 우회 재생하는 폴백을 적용했습니다.
- **🕒 알림 기록 데이터베이스 24시간 정리 주기 갱신**:
  * 디렉토리 내 축적되는 누적 로그 데이터의 부하를 덜고 효율적인 DB 관리를 위해, 기존 7일간 보존하던 알림 기록 자동 정리 주기를 **24시간(1일)**로 단축 갱신하였습니다.
- **🔍 장비 사전 데이터 교정 및 오류 픽스**:
  * 무기 '해머', '창(스피어)', '단검(대거)', '손목(스펠북)', '손목(펜듈럼)' 카테고리 내에서 실제 리소스 이름 불일치로 인해 이미지가 깨지거나 타 장비와 꼬여서 오매칭되어 있던 현상을 완벽하게 해결했습니다.
  * 改-세크리드 해머가 세크리드 해머로 중복 표기되던 명칭 오기를 정정하였습니다.
  * 세크리드 아밍소드 및 세크리드 리스트가 동일 계열의 정상 이미지(각각 cellImage_1073819565_6.jpg, cellImage_139432692_7.jpg)를 改 버전과 안전하게 공유하도록 연결했습니다.
  * 갑옷 '아노마라드'와 세트 '데모닉 장비 세트' 하위 데이터에 포함되어 장비 사전 탭의 가독성을 저해하던 설명 메모 텍스트 레코드들을 완전히 소거 정돈했습니다.
