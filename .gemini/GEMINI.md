...
- **리팩토링:** Scam Monitor 모듈 분리(Facade 패턴 적용) 및 XP Tracker 로직 역할 분리.
- **최적화:** GA4 이벤트 즉시 전송, 오프라인 무시, 체류 시간(Engagement Time) 증분 계산, 10분 주기 하트비트 전송, 글로벌 에러 트래킹 추가. 운영 환경에서의 불필요한 반복 로그 필터링 적용.

#### v1.12.4
- **수정:** 어벤던로드 트래커의 입장료 한글 포맷(예: "1억 40만 Seed") 및 띄어쓰기 예외("횟 수")를 정상 인식하도록 정규식을 고도화했습니다.
- **수정:** 어벤던로드 트래커의 상태 관리 아키텍처를 개편하여, 전역 ON/OFF(`isEnabled`)와 화면 표시 여부(`isActive`)를 분리했습니다. 계산기 창의 ON/OFF는 설정 파일에 저장되어 기능을 영구 제어하며, 즉시 숨김 버튼은 임시 화면 제어만 담당합니다.
