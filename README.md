...
![Version](https://img.shields.io/badge/version-1.19.1-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.19.1 (2026.06.26)
이번 업데이트는 **'사냥터 동선 시뮬레이션' 기능의 신규 사냥터 추가** 및 **맵 이미지 폴더명 변경(field-map)과 기존 사용자 데이터베이스 마이그레이션 안전장치**가 적용되었습니다.

- **🗺️ 사냥터 동선 시뮬레이션 신규 사냥터 추가**:
  * 기존 '시오칸하임 대장간' 외에 신규 사냥터인 **'골고다의 협곡'** 및 **'공허의 영역'** 동선 시뮬레이션 탭을 새롭게 추가하였습니다.
  * 사냥터 탭 표시 이름을 더 직관적으로 한눈에 알아볼 수 있도록 수정하였습니다 (예: 대장간 -> 시오칸하임 대장간).
- **📁 맵 이미지 오해 소지 폴더명 변경 및 마이그레이션**:
  * 맵 이미지가 저장되는 폴더명을 오해가 생기지 않도록 `field-macro`에서 `field-map`으로 변경하고 관련 경로들을 모두 업데이트했습니다.
  * 기존 사용자가 업데이트 시에도 대장간 이미지의 엑스박스 등 누락 오류가 없도록, 기존 로컬 DB(`diary.db`)에 설정된 이전 경로 및 명칭을 자동으로 최신 상태로 강제 전환해 주는 안전 마이그레이션 구문을 적용했습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.19.1.exe` 파일을 다운로드하여 실행하세요.
