...
![Version](https://img.shields.io/badge/version-1.13.6-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.13.6 (2026.05.24)
이번 업데이트는 **이자벨의 비법(비율 능력치) 자동 감지 기능 제거** 및 **중복 버프 그룹 덮어쓰기 로직 보완**을 진행한 핫픽스 버전입니다.

- **⏱️ 버프 감지 및 중복 버프 타이머 로직 개선**: 퇴마사의 은총과 시스템 로그를 공유하여 오감지 및 타이머 리셋 문제를 일으키던 이자벨의 비법 (비율) 버프의 자동 감지 트리거를 제거했습니다. 또한, 이자벨 비율과 퇴마사의 은총처럼 중복 사용이 불가능한 버프 그룹에 속한 버프가 감지되면, 기존 활성화된 버프 타이머가 자동으로 만료 처리되도록 개선하였습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.13.6.exe` 파일을 다운로드하여 실행하세요.
