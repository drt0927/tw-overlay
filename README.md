...
- **📅 달력 내 득템 스택 뷰 및 이미지 연동**:
  * 각 날짜 칸 하단에 그날 획득한 아이템들이 미니 아이콘 이미지와 수량이 표기된 스택 배지 형태로 깔끔하게 중복 합산되어 표현됩니다.
- **🔄 득템 키워드 설정 2컬럼 전송(Transfer List) UI 리뉴얼**:
  * 득템 키워드 설정을 위해 미등록/등록 리스트를 좌우로 분리하고, 실시간 검색 필터링과 기본값 복원, 대량 등록 및 해제 기능을 추가했습니다.
- **🛠️ 채팅 자동 수집 득템 정규화 및 합산 오류 수정**:
  * 채팅으로 감지되는 시스템 득템 로그를 정확히 해석하여 수동 입력과 동일하게 이름이 정규화되고 중복 수량이 합산되도록 개선했으며, 이미지 엑박 깨짐 오류와 타임라인 `x` 삭제 버튼의 정렬 문제도 바로잡았습니다.
