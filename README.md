...
![Version](https://img.shields.io/badge/version-1.17.6-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.17.6 (2026.06.06)
이번 업데이트는 채팅 오버레이에 인게임 NPC 및 몬스터 대사 표시 여부를 선택할 수 있는 설정 옵션을 추가하고, 클럽 시스템 공지(보스 소환/참가 등)가 지정 단어 알림으로 작동하여 발생하는 공해 현상을 수정하였습니다.

- **💬 인게임 NPC 및 몬스터 대사 필터 설정**: 채팅 오버레이 화면에 보스, NPC 및 몬스터의 대사를 표시하거나 숨길 수 있는 체크박스 옵션을 추가하고 필터 가이드를 제공합니다.
- **🐛 클럽 시스템 공지 단어 알림 제외**: '클럽 공지' 발신자가 작성하는 보스 소환 및 참가 확인 대사들이 지정 단어 알림(OS 토스트 알림 및 알림음)에 감지되지 않도록 완전히 차단했습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.17.6.exe` 파일을 다운로드하여 실행하세요.
