...
![Version](https://img.shields.io/badge/version-1.17.4-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.17.4 (2026.06.05)
이번 패치 업데이트는 시스템 채팅 로그에 포함된 단어에 의해 발생하는 비정상적인 알림 버그와, 특정 상황에서 데이터베이스 오류로 인해 채팅 실시간 수집 프로세스가 멈추던 문제를 해결하여 안정성을 크게 강화했습니다.

- **🐛 지정 단어 알림 및 디스코드 전송 시 시스템 메시지 제외**: 버프 사용 메시지 등 시스템이 남기는 채팅은 일반 및 클럽 지정 단어 알림 대상에서 제외되도록 수정하여 오알림을 차단했습니다.
- **⚡ 알림 삭제 시 발생하는 데이터베이스 오류 크래시 해결**: 지정 단어 알림이 발생한 직후 사용자가 알림을 즉시 삭제하는 경우, 후속 대화 수집 과정에서 외래 키(FOREIGN KEY) 제약 조건으로 인해 발생하던 데이터베이스 에러와 실시간 감시 엔진 멈춤 현상을 방지하는 예외 방어 처리를 적용했습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.17.4.exe` 파일을 다운로드하여 실행하세요.
