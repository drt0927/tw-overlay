...
![Version](https://img.shields.io/badge/version-2.0.6-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v2.0.6 (2026.07.04)
이번 업데이트는 커스텀 알림 화면 가로 확장, 알림별 개별 볼륨 슬라이더 추가, 설정창 바로가기 하이라이트 효과 및 1/2depth 탭 간헐적 먹통 현상 해결을 담은 **패치 v2.0.6 업데이트**입니다.

- **📢 커스텀 알림 화면 가로 확장 및 개별 볼륨 슬라이더 추가**:
  * 커스텀 알림 설정 창의 너비를 420px에서 580px로 키워 가독성을 대폭 높였습니다.
  * 생성/수정 폼 및 각 알림 카드 하단에 개별 볼륨 조절 슬라이더(0% ~ 100%)를 추가하여, 각 알림음 크기를 독립적으로 정밀하게 조절할 수 있습니다.
  * 미리듣기 버튼을 연속 클릭할 때, 기존 재생 중이던 소리를 즉각 정지하고 새 소리를 처음부터 다시 재생(인터럽트 재생)하게 처리했습니다.
- **✨ 설정창 대메뉴 기능별 알림 바로가기 하이라이트 기능**:
...
- **🐛 UI 버그 수정 및 개선**:
  * 설정창 1depth 탭 이동 시 2depth 서브 탭 본문이 간헐적으로 가려지거나 사라지던 버그를 완벽히 해결했습니다.
  * 버프 타이머 설정 창 내 사운드 드롭다운의 찌그러짐/삐짐 현상을 해결하고, 경험치 HUD 미리듣기 버튼 디자인 및 색상 일관성을 보정했습니다.
