...
![Version](https://img.shields.io/badge/version-1.18.14-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.18.14 (2026.06.26)
이번 업데이트는 **사미선령지채 버프 타이머 추가 및 버프 계산기 로직의 수치 구조화 리팩토링**이 적용되었습니다.

- **⏱️ 사미선령지채 버프 타이머 추가**:
  * 요리 스킬을 통해 제작 및 사용되는 '사미선령지채' (지속시간 5분, 모든 능력치 +30) 아이템의 사용 감지 및 타이머 연동이 새롭게 추가되었습니다.
- **⚙️ 버프 효과 데이터 구조화**:
  * `buffs.json` 내의 텍스트 기반 효과 정의를 각 수치별 필드로 구조화하여 데이터 가용성을 확보하고 연산 속도를 대폭 개선했습니다.
- **🛡️ 버프 계수 계산기 정규식 파싱 버그 수정**:
  * 기존 계산기에서 퍼센트 버프(예: 이자벨 비율 +10%) 파싱 시 고정 수치(+1)로 잘못 감지되던 정규식 중복 매칭 버그를 데이터를 직접 읽어오는 방식으로 리팩토링하여 원천 해결했습니다.
