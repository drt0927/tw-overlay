...
![Version](https://img.shields.io/badge/version-2.1.4-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v2.1.4 (2026.07.13)
이번 업데이트는 기믹 알람음 재생 오류 및 설정 화면의 비동기 로딩 꼬임 문제를 완벽히 해결하고 사용성 편의 카드를 보강한 **패치 v2.1.4 업데이트**입니다.

- **🎵 기믹 알림음 재생 꼬임 및 먹통 오류 해결**:
  * 기믹 알림음 연속 클릭 및 빠른 연타 시 크로미움 오디오 엔진의 재생 중단(`AbortError`) 예외와 프로토콜 캐시 버그로 인해 사운드가 더 이상 재생되지 않고 대기열이 영구히 고착되는 현상을 완전히 해결했습니다. (동적 Audio 객체 생성 방식 개편 및 캐시 버스팅 파라미터 적용)
- **⚙️ 설정 로딩 레이스 컨디션 해결**:
  * 환경설정 화면 진입 시 비동기 옵션 로딩 지연으로 인해 기믹 알람음 설정값들이 `사용 안 함 (소리 없음)`으로 오작동 및 강제 초기화되던 오류를 해결했습니다.
- **🔗 소리 및 알림 설정 탭 바로가기 카드 추가**:
  * 환경설정의 `소리 및 알림` 탭에서 `기믹 & 도전과제` 설정 구역으로 즉시 탭 이동할 수 있는 바로가기 단축 카드를 신설하여 설정 이동 편의성을 대폭 보강하였습니다.

*(이전 v2.1.3의 변경 사항인 기믹 알림음 및 볼륨 개별 설정 신설 등은 [release-note](release-note/) 폴더의 릴리즈 노트를 참조해 주세요.)*
