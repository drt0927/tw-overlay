...
![Version](https://img.shields.io/badge/version-1.17.0-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.17.0 (2026.06.04)
이번 업데이트는 게임 내에서 편리하게 대화 로그를 조회하고 카테고리별로 모아볼 수 있는 **인게임 채팅 오버레이 위젯**이 신규 도입되었으며, **에타(ETA) 레벨 배지 지원** 및 **사기꾼 탐지 기능의 테스트/안전성 보완**이 대거 적용된 마이너 업데이트 버전입니다.

- **💬 인게임 채팅 오버레이 위젯**: 게임 화면 상에 탭 분류(전체/일반/귓속말/팀/클럽/외치기/시스템)를 지원하는 투명 채팅창을 지원하며, 단축키(`Ctrl + Shift + T`) 마우스 클릭 투과 및 이전 로그 무한 스크롤이 탑재되었습니다.
- **🛡️ 에타(ETA) 레벨 배지 연동**: 실시간 귓속말이나 일반 대화 인입 시, `etaCacheManager`에 캐싱된 상대 유저의 실시간 에타 레벨 배지를 닉네임 옆에 표기하여 신뢰성을 강화합니다.
- **⚡ 대화 스캔 속도 초고속화**: 지연 파싱(Lazy Parsing) 아키텍처를 도입하여 앱 기동 시 대량 로그 복원 시의 렉 현상을 원천 방지했습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.17.0.exe` 파일을 다운로드하여 실행하세요.
