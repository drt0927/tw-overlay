...
![Version](https://img.shields.io/badge/version-1.17.11-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.17.11 (2026.06.11)
이번 업데이트는 채팅 오버레이에 콜론(`：`)을 포함한 사칭 의심 닉네임을 감지하여 경고 배지와 색상 강조 표시를 적용하는 사기 및 사칭 방지 기능을 추가했습니다.

- **🚨 사칭 의심 닉네임 강조 및 배지 표시**: 닉네임 내에 풀위드 콜론(`：`)이 들어간 사칭 의심 유저의 채팅을 감지하여 빨간 점선 테두리 및 `⚠️ 사칭주의` 배지를 표시해 한눈에 구별할 수 있도록 개선했습니다.
- **⚙️ 설정 제어 추가**: 환경 설정의 테일즈위버 채팅 로그 설정 영역에서 해당 강조 기능의 사용 여부를 쉽게 켜고 끌 수 있는 토글 옵션을 추가했습니다.
- **🧪 사칭 사기 테스트 시나리오 추가**: 사기꾼 탐지기(`scam-detector.html`) 테스트 케이스 목록에 특수문자 `：` 사칭 패턴을 가상으로 주입해 볼 수 있는 테스트용 대화 시나리오를 신설했습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.17.11.exe` 파일을 다운로드하여 실행하세요.
