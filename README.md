...
![Version](https://img.shields.io/badge/version-1.18.0-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.18.0 (2026.06.12)
이번 업데이트는 전투 장비 스펙 검색 및 비교가 가능한 **장비 사전**을 신규 출시하고, 이를 **계수 계산기 및 진화 계산기와 다이렉트 연동**했습니다. 또한 계수 계산기 레이아웃을 **와이드 2분할 디자인으로 리뉴얼**하고 공식 수식 오류를 정밀 보정했습니다.

- **📖 신규 '장비 사전' 윈도우 출시**: 한글 초성 검색(예: `ㅅㅋㄹㄷ` -> `세크리드`) 및 최대 4개 장비의 세부 스펙 비교 대조 기능을 제공합니다.
- **🔗 장비 사전 ➡️ 계산기 연동**: 사전의 상세 정보 카드에서 클릭 한 번으로 해당 장비를 계수 계산기에 주입하거나 진화 비용 계산기 단계를 자동 지정할 수 있습니다.
- **📐 계수 계산기 레이아웃 리뉴얼**: 화면을 가로 2분할 와이드 디자인(`1420x760`)으로 개편하여 가독성을 높이고, 콘텐츠 가이드 카드에 목표 대비 도달 비율 프로그레스 바를 탑재했습니다.
- **⚙️ 공식 보정 및 데이터 복구**: 마법베기 및 물리복합 코어 가중치를 정밀 보정하여 엑셀 공식과 100% 수치를 일치시켰고, 누락되었던 어비스 방어구 데이터를 복구했습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.18.0.exe` 파일을 다운로드하여 실행하세요.
