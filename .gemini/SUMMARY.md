# TalesWeaver Companion Browser (TW-Overlay) 프로젝트 가이드 (v1.6.5)

이 문서는 v1.6.5 버전을 기준으로 작성되었습니다.

## 1. 개요 및 최종 상태
- **목적:** 테일즈위버 전용 지능형 오버레이 브라우저 및 게임 동기화 위젯.
- **최종 빌드:** **TypeScript**, Electron, **Native Win32 API(Koffi)** 기반.
- **주요 개선:** 게임 종료 시 프로그램 유지 및 커스텀 리마인더 알림 기능 추가.
...
- **업데이트 히스토리 (v1.6.5 핵심)**
- **신규 기능:** 게임 종료 시 사용자가 설정해둔 리마인더 메시지를 커스텀 팝업 창으로 표시하는 기능 추가.
- **동작 변경:** 게임 종료 시 프로그램이 함께 종료되지 않고 백그라운드에서 유지되며, 게임 재시작 시 자동 재추적.
