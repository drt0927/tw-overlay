...
![Version](https://img.shields.io/badge/version-1.17.3-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.17.3 (2026.06.05)
이번 패치 업데이트는 윈도우 고해상도(DPI) 배율 환경에서 환경설정이나 채팅 오버레이 창을 활성화했을 때, 오버레이 크기가 우측으로 무한히 늘어나며 심한 렉을 유발하던 치명적인 버그를 수정했습니다.

- **🐛 고해상도 및 DPI 배율 환경에서 창 크기가 계속 늘어나는 현상 해결**: 프로그램에 의한 정렬 시 2px 이하의 미세한 변화는 무시하도록 임계값(Threshold) 검사를 도입하여, 픽셀 변환 반올림 오차로 인해 창 크기가 매 프레임 확장되던 문제를 차단했습니다.
- **⚡ 비동기 이동 정렬 레이스 컨디션 및 디스크 쓰기 병목 해결 (Timestamp Guard)**: 비동기적인 `move` 이벤트 다중 전파로 인해 자동 정렬 좌표가 어긋나며 매 밀리초 단위로 설정 파일을 디스크에 강제 저장하던 렉 유발 구조를 시간 가드(200ms)를 통해 원천 해결했습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.17.3.exe` 파일을 다운로드하여 실행하세요.
