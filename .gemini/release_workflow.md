# 📦 TW-Overlay 릴리즈 워크플로우 가이드

이 문서는 새로운 버전을 배포할 때 수행해야 하는 모든 단계(문서 업데이트, 코드 수정, 빌드 및 배포)를 정의합니다.

## 🏁 릴리즈 체크리스트

### 1. 코드 및 버전 정보 수정
- [ ] **package.json**: `version` 필드를 새 버전으로 업데이트.
- [ ] **src/main.ts**: 파일 상단 주석의 버전 정보 업데이트.
- [ ] **src/settings.html**: '앱 정보' 섹션의 `현재 버전` 텍스트 업데이트.
- [ ] **빌드 확인**: `npm run build`를 실행하여 컴파일 오류가 없는지 확인.

### 2. 프로젝트 문서 업데이트
- [ ] **.gemini/GEMINI.md**:
    - 문서 상단의 버전 정보를 최신화.
    - 새롭게 추가된 주요 로직이나 아키텍처 변경 사항 반영 (예: 빌드 시스템 변경, 스플래시 추가 등).
- [ ] **.gemini/SUMMARY.md**:
    - 프로젝트 요약 정보 최신화.
    - 기능 목록에 새로 추가된 기능 추가.
- [ ] **README.md**:
    - 메인 페이지의 버전 정보 및 최신 기능 소개 업데이트.

### 3. 릴리즈 노트 작성
- [ ] **release-note/CHANGELOG-vX.X.X.md**:
    - 신규 버전 전용 릴리즈 노트 파일 생성.
    - **Added**: 신규 기능.
    - **Changed**: 변경 및 개선 사항.
    - **Fixed**: 버그 수정 내역.
- [ ] **release-note/CHANGES.md**: 전체 변경 이력 요약본에 최신 버전 정보 추가.

### 4. 배포 및 태깅 (GitHub)
- [ ] **로컬 테스트**: `npm run dist`를 실행하여 생성된 `.exe` 설치 파일로 최종 검증.
- [ ] **Git 커밋**: "chore: release vX.X.X" 메시지로 변경 사항 커밋.
- [ ] **태그 생성**: `git tag vX.X.X` 실행.
- [ ] **푸시**: `git push origin main --tags` 실행. (GitHub Actions가 자동으로 빌드 및 드래프트 릴리즈 생성)

---

## 💡 1.0.7 버전 주요 변경 사항 (작업 필요 메모)
이번 1.0.7 버전 업데이트 시 다음 내용들을 문서에 반영해야 합니다:
- **성능 최적화**: 로컬 에셋(Tailwind, Lucide) 도입 및 창 생성 로직 개선.
- **업데이트 시스템**: GitHub 기반 자동 업데이트 확인 및 환경설정 내 통합 UI 구현.
- **UX 개선**: 스플래시 화면(Splash Screen) 추가 및 초기 구동 부하 분산 처리.
- **빌드 시스템**: PowerShell 기반 복사 로직을 Node.js 스크립트(`copy-resources.js`)로 전면 교체.
