...
### 5. 강제 업데이트(Mandatory Update) 설정 (선택)

강제 업데이트란 앱 실행 시 사용자가 거부할 수 없도록 즉시 다운로드 및 설치를 강제하는 업데이트입니다.  
보안 패치, 심각한 버그 수정, 서버 호환성 변경 등 모든 유저가 반드시 올려야 하는 버전에만 사용하세요.

**활성화 방법**: GitHub의 드래프트 릴리즈에서 다음 중 하나를 설정합니다.

- **방법 A** — Release Name(릴리즈 제목)에 태그 포함:
  ```
  [Mandatory Update] v2.0.3
  ```
- **방법 B** — 릴리즈 노트 본문 어딘가에 태그 포함:
  ```
  [Mandatory Update]
  ```

**동작 흐름**:
1. 앱 실행 시 `[Mandatory Update]` 태그 감지
2. 스플래시 화면 잠금 (사용자가 앱 사용 불가)
3. 업데이트 즉시 자동 다운로드
4. 다운로드 완료 후 1.5초 뒤 자동 설치 및 재시작

> **⚠️ 주의**: 강제 업데이트는 드래프트를 **Publish(게시)** 해야 실제 유저에게 적용됩니다.
