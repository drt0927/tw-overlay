...
![Version](https://img.shields.io/badge/version-1.17.2-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.17.2 (2026.06.05)
이번 패치 업데이트는 게임창 최소화 시 리소스 최적화를 위해 모든 유틸리티 창을 닫는 과정에서 채팅 히스토리 오버레이 창이 영구적으로 비활성화되는 버그를 수정하여 게임창 최소화/복원 시 이전 활성화 상태 그대로 오버레이가 정상 복원되도록 개선했습니다.

- **🐛 게임창 최소화 시 채팅 히스토리 오버레이 강제 비활성화 문제 해결**: 게임창 최소화 시 오버레이 창이 닫혀도 사용자의 수동 비활성화 동작과 구분하여 설정 값 및 상태 변수를 보존하도록 통제 흐름을 일원화했으며, 복원 시 이전 켜짐 상태를 감지하여 자동으로 다시 띄웁니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.17.2.exe` 파일을 다운로드하여 실행하세요.
