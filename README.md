...
![Version](https://img.shields.io/badge/version-1.11.2-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.11.2 (2026.04.22)
이번 업데이트는 사냥 데이터를 한눈에 분석할 수 있는 **지능형 시각화 도구**와 대규모 수치 처리의 **안정성**에 집중했습니다.

- **📈 실시간 사냥 리듬 차트**: 최근 30분간의 경험치 획득량을 그래프로 시각화합니다.
- **💎 경험의 정수 예측기**: 현재 사냥 속도 기준, 시간당 정수(100억 XP) 생산량을 실시간으로 알려줍니다.
- **💾 완벽한 데이터 보존**: 창을 닫았다가 다시 열어도 사냥 기록이 그대로 복구됩니다.
- **🛡️ 21억 오버플로 수정**: 경험치가 21억을 넘을 때 수치가 마이너스로 표시되던 버그를 완전히 해결했습니다.
- **🔌 오프라인 모드 최적화**: 인터넷 연결이 없어도 모든 차트와 기능을 사용할 수 있습니다.
...
### 📊 실시간 게임 데이터 분석 (신규)
- **[경험치 HUD (XP/EPM/Chart)](./docs/overlay.md)**: 실시간 획득 경험치, EPM, 사냥 리듬 차트 및 정수 기댓값 표시
- **[외치기 히스토리](./docs/trade.md)**: 실시간 외치기 수집 및 검색, 닉네임 원클릭 복사
- **[실시간 로그 엔진]**: 채팅 로그 실시간 추적을 통한 자동 일지 기록 및 득템 알림

### 🔔 알림 및 실시간 모니터링
- **[지능형 버프 타이머]**: 핵심 버프(심장류, 퇴마사)를 감지하여 뱃지 형태로 남은 시간을 보여줍니다.
