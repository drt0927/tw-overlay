...
![Version](https://img.shields.io/badge/version-1.18.8-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.18.8 (2026.06.20)
이번 업데이트는 **경험치 및 엘소(Elso) 습득 표시 선택 옵션 제공 및 설정 체크박스 UI 리팩토링**이 적용되었습니다.

- **⚙️ 경험치 및 엘소 습득 필터링 온/오프 기능**:
  - 환경 설정의 채팅 설정 탭 내에 경험치 및 엘소 습득 로그의 오버레이 표시 여부를 설정할 수 있는 온/오프 토글을 추가했습니다.
  - **엘소 습득**: 오직 `[엘소 N포인트]을(를) [K]개 획득하였습니다.` 형태의 드랍/득템 로그만 조절하여 채팅 도배를 막고, 사용자가 직접 더블 클릭하여 충전한 액션 로그는 피드백 제공을 위해 필터와 무관하게 항상 노출되도록 필터링을 고도화했습니다.
- **🎨 설정 화면 체크박스 UI 통일화**:
  - 채팅 설정 탭 내의 모든 체크박스(통합 채널 선택, NPC 필터, 사칭 의심 닉네임 강조 등)를 카드형 레이아웃과 둥근형 디자인 시스템 표준 규격으로 일괄 리팩토링하여 디자인 완성도를 높였습니다.
