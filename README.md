...
![Version](https://img.shields.io/badge/version-2.0.4-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v2.0.4 (2026.07.04)
이번 업데이트는 계수 계산기 사용성 보완 및 Z-Order 정렬 개선, 그리고 버그 픽스를 담은 **패치 v2.0.4 업데이트**입니다.

- **🧮 계수 계산기 내 DEX 명칭 '명중'으로 완전 일원화**:
  * 테이블, 요약 정보, 상세 보너스 툴팁, 콘텐츠 가이드 등 계수 계산기 내부의 모든 DEX 영어 표기를 친숙한 한국어인 '명중'으로 통합하여 가독성을 높였습니다.
- **🧮 도핑 상세 보너스 툴팁 줄바꿈 방지 및 수동 ON/OFF 추가**:
  * 보너스 툴팁의 뒷배경 텍스트가 겹치지 않게 어둡고 고급스러운 불투명 배경을 도입했으며, `'찌르기:'`가 줄바꿈되어 잘리는 현상을 수정했습니다.
  * 도핑 선택 셀렉터 옆에 `툴팁 ON/OFF` 텍스트형 토글 버튼을 추가해 언제든 툴팁의 가시 상태를 제어할 수 있도록 UX를 개선했습니다.
- **🛡️ 서브 창 간 포커스 시 Z-Order 정상화**:
  * 설정, 계산기 등 다중 서브 창 사용 시 현재 포커싱된 창이 다른 창에 가려지지 않고 항상 최상단에 안정적으로 위치하도록 개선했습니다.
- **🔔 미리보기 테스트 토스트 연동 및 오버레이 설정 보강**:
  * 사운드 미리보기 테스트 시에도 설정 상태에 맞추어 오버레이 토스트가 잘 렌더링되게 고치고, 사이드 모드일 때 토스트를 게임 오버레이에 띄울지 제어하는 설정을 추가했습니다.
