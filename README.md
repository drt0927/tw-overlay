...
![Version](https://img.shields.io/badge/version-1.13.1-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.13.1 (2026.05.17)
이번 업데이트는 **'시에나의 기운 시뮬레이터' 기능 고도화 및 UI/UX 개선**을 주요 목표로 합니다.

- **⚡ 즉시 단계 설정 추가**: 재화 소모 없이 3/7/10단계로 즉시 이동하여 옵션을 테스트할 수 있는 기능이 추가되었습니다.
- **📈 탭별 독립 대시보드 및 통계 탭 신설**: 증폭, 능력치, 추가 옵션 각각의 특성에 맞는 대시보드를 제공하며, 전체 소모 내역을 한눈에 보는 '통계' 탭이 추가되었습니다.
- **🔍 개별 슬롯 비용 추적**: 추가 옵션의 각 슬롯별(1, 2, 3) 누적 소모 비용을 독립적으로 확인하고 초기화할 수 있습니다.
- **⚙️ 논리적 초기화 시스템**: 환류의 서(전체)와 정환의 서(단일)의 동작 방식을 반영하여 더 직관적인 초기화 로직을 구현했습니다.
- **📱 가독성 및 안정성 개선**: 폰트 크기 상향 및 엘소(ELSO) 비용 고정 너비 표시(N만 XXXX)를 통해 자동 강화 시 UI 깜박임을 해결했습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.13.1.exe` 파일을 다운로드하여 실행하세요.
