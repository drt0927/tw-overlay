...
![Version](https://img.shields.io/badge/version-1.17.5-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.17.5 (2026.06.05)
이번 패치 업데이트는 필드보스 알림 설정에 "혼란한 대지" 및 스폰 시간대 추가와, 다중 필드보스 출현 시 알림이 누락되지 않도록 로직을 개선하고 알림음 겹침 현상을 해결하기 위해 순차 재생 큐 시스템을 적용했습니다.

- **🔔 필드보스 알림에 "혼란한 대지" 추가**: 스폰 시간대(00시, 07시, 13시, 18시, 21시)를 추가하고 관련 설정을 지원합니다.
- **⚡ 동시간 다중 알림 누락 방지 및 순차 재생**: 여러 보스가 겹쳤을 때 알림이 누락되는 현상을 수정하고, 알람음이 겹쳐서 끊기지 않고 0.5초 간격으로 차례대로 들리도록 순차 재생 큐를 도입했습니다.
- **🐛 필드보스 설정 UI 버그 수정**: 설정 페이지에서 '혼란한 대지'의 스폰 시간이 정상적으로 표시되지 않던 데이터 바인딩 버그를 해결했습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.17.5.exe` 파일을 다운로드하여 실행하세요.
