# TalesWeaver Companion Browser (TW-Overlay) 프로젝트 가이드 (v1.8.1)

이 문서는 v1.8.1 버전을 기준으로 작성되었습니다.
...
- **분석 강화:** 앱 사용 지속 시간을 추정하기 위한 Heartbeat(Ping) 핑 기능이 도입되었습니다.
...
### 7. 게임 종료 리마인더 (Game Exit Reminder)
- **프로그램 유지**: 게임 종료 시 `app.quit()` 대신 모든 창을 숨기고 폴링을 계속하여, 게임 재시작 시 자동으로 다시 추적합니다.
- **커스텀 알림 창**: 게임 종료를 감지하면 사용자가 설정해둔 리마인더 메시지를 `game-exit-reminder.html` 커스텀 팝업으로 표시합니다.
- **미완료 숙제 연동**: (v1.7.0) 게임 종료 시 아직 완료하지 않은 숙제 목록을 추출하여 팝업창에 함께 표시합니다.

### 8. ETA 랭킹 조회 시스템 (ETA Ranking)
- **실시간 조회**: 외부 API를 통해 테일즈위버 ETA 랭킹 데이터를 실시간으로 가져와 사용자에게 제공합니다.
- **검색 및 필터링**: 캐릭터명이나 특정 조건으로 랭킹 내 데이터를 빠르게 검색할 수 있는 고성능 UI를 구현했습니다.

### 9. 숙제 체크 리스트 시스템 (Contents Checker)
- **지능형 초기화**: 테일즈위버의 실제 초기화 시각(00:00, 월요일 06:00 등)을 준수하는 자동 초기화 엔진을 탑재했습니다.
- **개인화 관리**: 카테고리 사용자화, 항목 숨기기, 커스텀 항목 추가 등 사용자별 맞춤 설정 기능을 제공합니다.
- **드래그 앤 드롭 순서 변경**: (v1.7.3) 관리 모드에서 마우스 드래그로 숙제 순서를 자유롭게 조정할 수 있으며, 드래그 시 자동 스크롤을 지원합니다.
- **하이브리드 섹션 뷰**: (v1.7.3) 조회 모드에서 '미완료 일일/주간'과 '완료 통합' 섹션을 분리하여 가독성을 극대화했습니다.
- **실시간 백그라운드 체크**: 창이 닫혀 있어도 매 분마다 초기화 시간을 체크하여 정각에 즉시 데이터를 갱신합니다.

### 10. 구글 애널리틱스 분석 강화 (Analytics & Heartbeat)
- **리텐션 측정 (Heartbeat)**: 사용자가 앱을 얼마나 오래 켜두는지 정확히 측정하기 위해 v1.8.1에서 **1시간 주기 Heartbeat(app_running_ping)** 기능이 도입되었습니다.
- **예외 처리**: 네트워크 오프라인 상태에서도 전송 시도가 앱에 영향을 주지 않도록 예외 처리가 강화되었습니다.
