...
![Version](https://img.shields.io/badge/version-1.18.7-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.18.7 (2026.06.19)
이번 업데이트는 **캐시 경험치 버프 3종(전설의 군고구마, 얼리버드 경험치 부스터, 최상급 에오스의 파편)의 중복 적용 오류 수정**이 적용되었습니다.

- **🛡️ 캐시 버프 중복 오류 수정**: 캐시 버프 아이템인 "전설의 군고구마"와 "최상급 에오스의 파편"의 버프 그룹을 `none`으로 수정하여, 게임 내에서 일반 버프(경험의 심장, 일반 에오스의 파편 등)와 함께 중복해서 사용하더라도 HUD 상에서 버프 타이머가 강제로 지워지던 문제를 해결하고 5초 전 만료 알림이 정상적으로 울리도록 수정했습니다.
- **📝 버프 설명 최신화**: 캐시 버프 3종의 설명을 게임 내 실제 획득 방식에 맞추어 "캐시 상품"으로 업데이트하였습니다.
