...
![Version](https://img.shields.io/badge/version-1.18.4-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.18.4 (2026.06.18)
이번 업데이트는 **경험치 HUD 초기화 단축키 추가 및 버프 타이머 비우기 개선**과 **신규 경험치 버프 3종 감지 추가**가 적용되었습니다.

- **⌨️ 경험치 세션 초기화 단축키 추가**: 전역 단축키 `Ctrl + Shift + X` 입력을 통해 경험치 HUD 데이터를 한 번에 리셋할 수 있는 편의 기능이 추가되었습니다.
- **🧹 버프 타이머 전체 삭제 버튼 및 단축키 추가**: `Ctrl + Shift + E` 단축키 혹은 설정 화면 내 [전체 삭제] 버튼으로 활성화 중인 버프 타이머를 일괄 삭제하는 기능이 추가되었습니다.
- **🧪 신규 경험치 버프 3종 감지 및 미리보기 보완**: 최상급 에오스의 파편, 전설의 군고구마, 얼리버드 경험치 부스터가 감지 대상으로 연동되며, 테스트 미리보기 실행 시 그룹 중복 제거로 인해 일부 버프가 누락되던 문제를 수정했습니다.
