...
![Version](https://img.shields.io/badge/version-2.1.5-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v2.1.5 (2026.07.16)
이번 업데이트는 테시스 코어 시스템의 강화 및 진화에 소모되는 재료와 비용의 기댓값을 계산해 주는 **테시스 코어 계산기** 기능이 신설된 **패치 v2.1.5 업데이트**입니다.

- **🧮 테시스 코어 계산기 신설**:
  * 각 조각별 0진화 0단계부터 4진화 4단계까지의 강화 및 진화 단계별 가루, 결정, 시드 및 엘소의 소모 기댓값을 정밀하게 계산해 줍니다.
  * **주스탯 / 부스탯**: 부스탯 강화 시 소모량이 50% 절감되는 룰이 완벽히 적용됩니다.
  * **할인율 바로가기**: 기본 0% 할인율에 더해 20% 이벤트 할인 적용 버튼 및 초기화 버튼이 탑재되어 있습니다.
- **📦 상자 구매 비용 합산 계산기 탑재**:
  * 가루/결정 조달을 위한 코어 상자 구매 가격(만 시드 단위) 및 수량을 수동 입력하여, 강화 기댓값 비용과 더한 최종 실질 총비용(시드 혹은 엘소+시드 혼합)을 정밀하게 산출해 줍니다.
- **👁️ UI 가독성 및 테이블 편의 최적화**:
  * 테이블 헤더(th) 고정을 통해 리스트가 길어져도 헤더가 상단에 유지됩니다.
  * 열 가로폭 배치를 최적화하여 단계 기댓값 영역의 긴 문구가 잘리지 않도록 개선하였습니다.

*(이전 v2.1.4의 변경 사항인 사운드 재생 오류 해결 등은 [release-note](release-note/) 폴더의 릴리즈 노트를 참조해 주세요.)*
