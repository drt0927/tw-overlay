...
![Version](https://img.shields.io/badge/version-1.15.0-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.15.0 (2026.06.02)
이번 업데이트는 **디스코드 키워드 알림 감지 설정 단순화 및 외치기 대화 연동 추가, 실시간 즉시 저장(Auto-save) 시스템**을 도입한 고도화 버전입니다.

- **💬 디스코드 알림 감지 설정 단순화**: 감지 범위를 기존 3종에서 '일반 대화 (나 포함)' 및 '클럽 대화' 2종으로 단순화하고 설정의 직관성을 개선했습니다.
- **📢 외치기 대화(Shout) 감지 추가**: 외치기 대화 옵션을 추가하여, 본인 혹은 타인이 확성기를 썼을 때의 매칭 단어도 디스코드 웹훅 알림으로 받을 수 있습니다.
- **⚡ 실시간 즉시 자동 저장**: 수동 저장 버튼을 없애고 추가/삭제/토글 변경이 일어나는 즉시 설정 파일에 기록되는 실시간 자동 저장을 도입하여 사용성을 한층 끌어올렸습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.15.0.exe` 파일을 다운로드하여 실행하세요.
