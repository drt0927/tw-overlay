...
![Version](https://img.shields.io/badge/version-1.13.5-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.13.5 (2026.05.24)
이번 업데이트는 **퇴마사의 은총 버프 사용 시 이자벨 비율 버프 오감지 문제 해결** 및 **중복 버프 그룹 덮어쓰기 로직 보완**을 진행한 핫픽스 버전입니다.

- **⏱️ 퇴마사의 은총 감지 및 중복 버프 타이머 로직 개선**: 퇴마사의 은총 사용 직후 뒤따르는 '비아누의 숨결' 시스템 메시지에 의해 이자벨 비율 버프가 오감지되는 버그를 수정했습니다. 또한, 이자벨 비율과 퇴마사의 은총처럼 중복 사용이 불가능한 버프 그룹에 속한 버프가 감지되면, 기존 활성화된 버프 타이머가 자동으로 만료 처리되도록 개선하였습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.13.5.exe` 파일을 다운로드하여 실행하세요.
