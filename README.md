...
![Version](https://img.shields.io/badge/version-2.1.1-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v2.1.1 (2026.07.09)
이번 업데이트는 사냥터 동선 시뮬레이션 인게임 오버레이 모드 고도화 및 윈도우 관리 안정성을 향상시킨 **패치 v2.1.1 업데이트**입니다.

- **🗺️ 사냥터 동선 시뮬레이션 오버레이 모드 고도화**:
  * 오버레이 모드 활성화 시 노드 원형 및 순서 숫자를 모두 숨기고 오직 선과 화살표만 깔끔히 노출하도록 렌더링을 최적화했습니다.
  * 미니 설정 패널 헤더 드래그를 통해 오버레이 창 위치를 화면 상에서 자유롭게 변경할 수 있도록 이동 기능을 제공합니다.
  * 맵 조절 스케일을 1% 단위로 미세 조작할 수 있게 개선하고, 맵 축소율에 맞춰 선 두께와 화살표 머리 크기까지 비례해서 동조 축소되도록 자동 보간했습니다.
- **🖥️ 윈도우 라이프사이클 및 포커스 복원 보강**:
  * 오버레이 모드 해제(편집 복귀) 즉시 창이 타 앱이나 게임 화면 뒤로 파묻히지 않도록 포커스 강제 복구 처리를 적용했습니다.
  * Electron 프로세스에서 창이 파괴된 후 예외가 발생하는 것을 방지하기 위해 `isDestroyed()` 유효성 검사를 일괄 보강했습니다.
