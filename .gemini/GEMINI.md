# TalesWeaver Companion Browser (TW-Overlay) 프로젝트 가이드 (v1.12.6)

이 문서는 v1.12.6 버전을 기준으로 작성되었습니다.
...
- **수정:** 모험일지 타임라인 시간 표시 포맷 개선 — 'HH:mm' 형식으로 정갈하게 표시 (DB 원본은 초 단위 보존).
- **수정:** 사이드바 노출 안정성 강화 — 게임 비활성 상태에서 알람 발생 시 사이드바가 나타나는 버그 수정.
