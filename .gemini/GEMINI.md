...
- **비동기 데이터 바인딩:** 설정 화면 등에서 동적 UI 생성 시 데이터가 유실되지 않도록 `lastConfig` 보관 및 지연 바인딩 로직을 적용했습니다.

### 5. 업데이트 히스토리 (v1.10.3 핵심)
- **독립 유틸리티 모드:** 게임이 꺼져 있거나 최소화되어도 트레이 메뉴를 통해 도구(계산기, 다이어리 등)를 독립적으로 사용 가능합니다.
- **트레이 메뉴 확장:** 사이드바의 모든 기능을 트레이 아이콘 우클릭 메뉴에 통합했습니다.
- **모험 일지 통계:** 월간 출석 히트맵, 최애 보스 Top 3, 요일별 활동 리듬 등 인포그래픽 통계 탭을 추가했습니다.

### 6. 업데이트 히스토리 (v1.10.1 핵심)
- **버그 수정:** 환경 설정 내 '사이드바 메뉴 관리' 체크박스 데이터 바인딩 오류 수정.
- **UI 개선:** 필드보스 알림 설정 내 볼륨 슬라이더 및 시간 배지 가독성 향상.
