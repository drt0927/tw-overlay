...
![Version](https://img.shields.io/badge/version-1.13.4-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.13.4 (2026.05.23)
이번 업데이트는 **지능형 숙제 자동 체크 시스템 신설**과 **신규 버프 타이머 5종 추가 및 편의성 개선**을 주요 목표로 합니다.

- **📅 지능형 숙제 자동 체크 시스템 신설 및 커스텀 숙제 고도화**: 게임 내 실시간 채팅 로그를 자동으로 파싱하여 `베스티지`, `아페티리아 (일반/어려움)`, `이클립스`, `머큐리얼`, `어비스 코어 마스터` 등 약 20여 종의 핵심 던전 클리어를 스스로 감지하고 완료 횟수를 자동으로 카운트해 주는 획기적인 자동 체크 시스템이 신설되었습니다. 이와 함께 커스텀 주간 숙제 등록/수정 시 최대 수행 횟수(maxCount) 설정 기능을 완전 연동하고 한도 초과 시 자동 보정 안전 장치를 설계했습니다.
- **⏱️ 신규 버프 타이머 5종 추가 및 제어 개선**: 유용하게 사용되는 로토의 부적, 속성 앰플, 이자벨의 비법(데미지/비율능력치), 일루미네이션 축제 음료 등 5종의 신규 버프 이미지와 잔여 시간 타이머를 추가하였습니다. 또한, 사용 중인 버프 타이머를 직접 클릭하여 수동으로 비활성화할 수 있는 기능을 구현하였습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.13.4.exe` 파일을 다운로드하여 실행하세요.
