...
- **⚡ 챗로그 경로 설정 즉시 인식 및 재연결**:
  * 설정에서 챗로그 경로를 선택한 뒤 **[인식 시작]**을 누르면 앱을 재기동하지 않고도 즉시 챗로그 탐색을 재시작합니다.
- **⚠️ 11개 챗로그 의존 기능 가이드 배너 및 실시간 연동**:
  * 챗로그를 사용하는 11개 창에 대해 경로가 비정상적일 때 최상단 경고 가이드를 표시하며, 설정 변경 후 인식이 시작되면 켜져 있던 모든 창들의 가이드 배너가 실시간으로 즉시 제거됩니다.
- **🎛️ 브라우저 오버레이 드래그 핸들 전체 툴바 확장**:
  * 브라우저 오버레이 창을 이동할 때 협소한 핸들 대신, 상단 툴바 전체 영역을 드래그하여 훨씬 편리하게 창을 이동할 수 있습니다.
- **🛠️ 경험치 세션 및 어벤던로드 리셋 로직 분리**:
  * 경험치 HUD 초기화 시 어벤던로드 현황까지 날아가던 문제를 수정하고, 마정석 계산기 내부에 전용 초기화 버튼을 추가했습니다.
