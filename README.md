...
![Version](https://img.shields.io/badge/version-1.17.1-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.17.1 (2026.06.04)
이번 업데이트는 버프 만료 임박 시 시각적인 강조 효과를 강력하게 더해주는 **화면 중앙 대형 만료 경고 오버레이**가 신규 도입되었으며, 유동적인 버프 테스트 시뮬레이션 개선 및 이자벨 비율 버프 잔재 코드 제거 등 안정성 강화가 적용된 패치 업데이트 버전입니다.

- **📢 화면 중앙 대형 만료 경고 오버레이**: 버프 만료 5초 전에 화면 한가운데에 약 2.3초간 콤팩트하고 강렬한 대형 경고 카드를 노출하여 버프 리필 타이밍을 정확히 인지하도록 돕습니다.
- **🥞 알림 중첩 스택(Stack) 아키텍처**: 여러 개의 버프가 동시에 만료되는 상황에서도 카드가 겹치지 않고 세로로 정렬되어 누적되며, 2.3초 후 자동으로 소멸(소멸 시 아래 카드들이 위로 슬라이드 업 정렬)되는 매끄러운 UX를 보장합니다.
- **⚙️ 전역 대형 알림 설정**: 복잡한 개별 토글 조작을 롤백하여 UI 복잡도를 낮추고, 설정 창 우측 시각 알림 섹션에 "화면 중앙 대형 알림" 스위치 단 하나로 일괄 ON/OFF 할 수 있게 간소화했습니다.
- **⚡ 실시간 테스트 연동**: 설정을 저장하지 않은 임시 상태에서 미리보기를 하더라도, 조작 상태가 즉시 반영되어 시뮬레이션이 동작하도록 테스트 연동성을 개선했습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.17.1.exe` 파일을 다운로드하여 실행하세요.
