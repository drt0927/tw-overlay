...
![Version](https://img.shields.io/badge/version-1.17.10-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.17.10 (2026.06.11)
이번 업데이트는 날짜 변경 및 재접속 시 채팅 데이터의 흐름을 개선하고, 사기꾼 탐지 모니터의 1:1 대화방 감지 시 창 자동 열기 및 UI 스타일 오류를 해결했습니다.

- **💬 날짜 변경 및 게임 재기동 시 채팅 유실/꼬임 해결**: 자정이 지나 오늘 날짜의 로그 파일이 새로 생성되거나 게임 재접속 시, 이전 날짜의 채팅 버퍼가 꼬이지 않고 오늘 로그를 정확히 로딩하여 갱신할 수 있도록 변경했습니다.
- **🚨 1:1 대화 로그 감지 시 사기꾼 탐지 창 자동 오픈**: 실시간 탐지 작동 중 새로운 1:1 대화창(파일)이 생성되면 사기꾼 탐지 창을 강제로 팝업하고 화면에 포커스합니다.
- **🎨 사기꾼 탐지 토글 스위치 UI 디자인 복구**: 내부 설정에서 토글 버튼 스타일이 적용되지 않아 브라우저 기본 체크박스로 표시되던 문제를 세련된 디자인 스위치로 복구했습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.17.10.exe` 파일을 다운로드하여 실행하세요.
