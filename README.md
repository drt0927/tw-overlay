...
![Version](https://img.shields.io/badge/version-2.0.7-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v2.0.7 (2026.07.06)
이번 업데이트는 채팅 오버레이가 화면 영역 바깥으로 이탈해 사라지는 현상을 복구하는 로직을 담은 **패치 v2.0.7 업데이트**입니다.

- **💬 채팅 오버레이 화면 이탈 복구 로직 추가**:
  * 채팅 오버레이 창(`chatOverlay`, `chatOverlaySub`, `chatOverlaySub2`)이 화면 영역 바깥으로 밀려나 완전히 사라져 보이지 않는 경우를 방지하기 위해, 오버레이 창이 열릴 때 화면 내에 있는지 검사하고 화면 밖에 위치하는 경우 활성화된 화면의 중앙으로 자동 복구시키는 안정화 로직을 도입했습니다.

*(이전 v2.0.6의 주요 변경 사항인 커스텀 알림 화면 확장, 알림 개별 볼륨 슬라이더 기능 등은 [release-note](release-note/) 폴더의 릴리즈 노트를 참조해 주세요.)*
