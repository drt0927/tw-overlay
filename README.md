...
![Version](https://img.shields.io/badge/version-1.13.0-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.13.0 (2026.05.16)
이번 업데이트는 **'시에나의 기운 시뮬레이터' 신규 추가 및 자동화 편의 기능 구현**을 주요 목표로 합니다.

- **🆕 시에나의 기운 시뮬레이터 추가**: 증폭, 능력치 재설정, 추가 옵션 등 모든 강화 단계를 시뮬레이션할 수 있는 전용 도구가 새롭게 추가되었습니다.
- **⚙️ 자동 재설정(Auto Reset) 기능**: 원하는 능력치나 추가 옵션(등급 포함)이 나올 때까지 자동으로 재설정을 반복하여 최적의 강화 전략을 지원합니다.
- **📊 정밀한 확률 및 기댓값 리포트**: 한국/일본 서버 공식 데이터를 기반으로 한 정확한 확률을 제공하며, 목표 달성까지의 **평균 예상 시도 횟수**를 계산해줍니다.
- **🛡️ 중복 옵션 방지 안전장치**: 불가능한 옵션 설정을 사전에 감지하고 차단하여 재화 낭비를 방지합니다.
- **📱 UI 가독성 최적화**: 3단 패널 레이아웃 확장 및 히스토리 자동 줄바꿈을 통해 더 쾌적한 사용 환경을 구축했습니다.
...
### 🧮 전문 계산기 및 시뮬레이터
- **[시에나의 기운 시뮬레이터](./docs/siena-aura.md) (v1.13.0 Hot)**: 증폭, 능력치 재설정, 추가 옵션 시뮬레이션 및 자동 설정 기능
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.13.0.exe` 파일을 다운로드하여 실행하세요.
