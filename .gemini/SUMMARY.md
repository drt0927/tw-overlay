# TalesWeaver Companion Browser (TW-Overlay) 프로젝트 가이드 (v1.8.0)

이 문서는 v1.8.0 버전을 기준으로 작성되었습니다.

## 1. 개요 및 최종 상태
- **목적:** 테일즈위버 전용 지능형 오버레이 브라우저 및 게임 동기화 위젯.
- **최종 빌드:** **TypeScript**, Electron, **Native Win32 API(Koffi)** 기반.
- **주요 개선:** Contents Checker 드래그 앤 드롭 순서 변경 및 하이브리드 섹션 뷰 도입.

## 2. 주요 기능 명세
- **Evolution Material Cost Calculator (New):** 장비 및 무기 진화에 필요한 소모 재료의 단가를 입력하여 총 시드 및 엘소 비용을 한 눈에 시뮬레이션 할 수 있는 전용 계산 창 추가.
- **GA4 Analytics (New):** Measurement Protocol 기반의 안정적이고 가벼운 자체 이벤트 집계 기능 (app_open, feature_used 등).
