...
![Version](https://img.shields.io/badge/version-1.16.0-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.16.0 (2026.06.04)
이번 업데이트는 창모드 전체화면 및 단일 모니터 사용 유저를 위한 **macOS 스타일 하단 Dock 런처(독바) 기능**이 새롭게 추가된 마이너 업데이트 버전입니다.

- **🖥️ macOS 스타일 하단 Dock 런처**: 단축키(`Ctrl + Shift + D`) 혹은 마우스 클릭으로 간편하게 여닫을 수 있는 하단 런처바를 탑재하여 오버레이 창 관리를 혁신적으로 간소화했습니다.
- **🎨 테마 컬러 포커싱 및 네온 글로우**: 각 아이콘 고유의 색상에 반응하는 호버 카드 발광 효과 및 3구역 정렬(일반/퀵슬롯/시스템) 가변 레이아웃이 적용되었습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.16.0.exe` 파일을 다운로드하여 실행하세요.
