...
![Version](https://img.shields.io/badge/version-1.18.13-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.18.13 (2026.06.25)
이번 업데이트는 **모험일지에 기록되는 경험의 정수 획득 경로 다양화(직접/자동 교환 및 퀘스트/아이템 보상)와 사냥 HUD 세션 통계 분리**가 적용되었습니다.

- **⚡ 경험의 정수 획득 감지 다양화**:
  * 기존 경험치 감소 형태의 직접/자동 교환 감지는 물론, 보급품 탈환 성공 등 퀘스트 보상이나 아이템 드랍을 통해 획득하는 다양한 형태의 경험의 정수 채팅 로그를 완벽히 감지하도록 개선했습니다.
- **🔄 HUD 세션 통계 및 모험일지 기록 분리**:
  * 사냥 중 직접/자동 교환된 경험의 정수는 경험치 HUD 세션 카운트와 모험일지 양쪽 모두 누적됩니다.
  * 반면, 퀘스트나 드랍을 통한 획득은 사냥 세션 통계(정수 예측기 등)에 영향을 주지 않고 오직 모험일지 타임라인에만 득템 기록으로 분리 저장됩니다.
- **🛡️ 중복 감지 방지 필터 적용**:
  * 자동 교환 버프 시 발생하는 중복 로그에 대해 필터를 적용하여 1회만 기록되도록 안전 장치를 마련했습니다.
