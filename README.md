...
![Version](https://img.shields.io/badge/version-1.17.12-violet.svg)

## 📸 스크린샷

![App Screenshot](./screenshot/screen1.png)

## 🚀 최신 버전: v1.17.12 (2026.06.16)
이번 업데이트는 채팅 채널별 글씨색 사용자 설정 기능 제공, 일일 숙제 '클럽던전' 추가, 오를리 방어전 자동 감지 및 숙제 카운팅 연동, 그리고 통찰의 비약 버프 타이머 등록을 포함합니다.

- **🎨 채팅 채널별 글씨색 설정 지원**: 인게임 채팅 오버레이 채널별(일반, 귓속말, 팀, 클럽, 외치기, 시스템) 글씨색을 Pickr 라이브러리를 통해 다크 테마에 어울리게 변경할 수 있습니다.
- **📋 일일 숙제 '클럽던전' 추가**: 숙제 체크리스트에 '클럽' 카테고리의 일일 숙제인 '클럽던전' 항목을 추가하였습니다.
- **🛡️ 오를리 방어전 자동 감지 연동**: 오를리 방어전 지옥 난이도 클리어 시스템 메시지를 자동으로 분석하여 주간 숙제 카운팅을 1씩 올릴 수 있게 연동하고 배지를 표출했습니다.
- **🧪 통찰의 비약 버프 타이머 및 이미지 매핑**: 통찰의 비약(대: 5분, 특대: 1분) 사용 시 버프 타이머가 정상 동작하도록 등록하고 아이콘 이미지를 매핑했습니다.
...
### 설치 방법
[Releases](https://github.com/drt0927/tw-overlay/releases) 페이지에서 최신 버전의 `twOverlay-Setup-1.17.12.exe` 파일을 다운로드하여 실행하세요.
