# TalesWeaver Companion Browser (TW-Overlay) 프로젝트 가이드 (v1.6.4)

이 문서는 v1.6.4 버전을 기준으로 작성되었습니다.
...
- **Overlay Freedom (New):** 오버레이 창이 게임 창 영역 밖으로도 자유롭게 이동할 수 있도록 제한 해제.
...
- **업데이트 히스토리 (v1.6.4 핵심)**
- **신규 기능 업데이트:** 계수 및 명중 계산기 화면 내 정보 출처 링크(참고자료) 3종 추가.
- **신규 기능:** 스탯/장비/버프를 종합하여 딜량과 명중을 시뮬레이션할 수 있는 '계수 및 명중 계산기' 추가.
- **시스템/최적화:** 앱 종료 시 폴링 루프 및 이벤트 리스너 메모리 누수 완벽 차단 로직(cleanupAllListeners) 추가.
- **버그 수정:** 서브 모니터 환경에서 오버레이 좌표가 어긋나는 스케일링 버그 수정 및 창 포커스 깜빡임 무한 루프(Debounce) 방지.
- **보안 및 안정성:** IPC URL 검증 및 네이티브 DLL 로드 실패 대비 예외 처리(Mock) 적용.
